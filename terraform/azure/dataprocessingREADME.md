...
```Bash
confluent flink connection create openai-connection --cloud Azure --region westeurope --type openai --endpoint https://api.openai.com/v1/chat/completions --api-key  $OPENAI_API_KEY --environment $TF_VAR_envid
```

Check if connection was created:

```bash
confluent flink connection  list --cloud Azure --region westeurope  --environment $TF_VAR_envid
```
...
You can double-check the Flink Pool CFU usage. In my case I am running right now 8 of 10 CFUs and have 8 Jobs running (use the Filter), number is visible after scroll down.
...
back to [Deployment-Steps Overview](README.md) or continue with Sink Cloud Services [azure-storage](azure-storage/README.md)